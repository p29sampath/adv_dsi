...
def eval_model(classifier, X_train, y_train, X_val, y_val, threshhold=0.5, show=True):
  """Evaluate Model:
  ...
  roc_score_val, acc_val, f1_val, precision_val, recall_val, r2_val = get_performance(model, X_val, y_val, "Validate",threshhold, show)  
  roc_score_training, acc_train, f1_train, precision_train, recall_train, r2_train  = get_performance(model, X_train, y_train, "Train",threshhold, show)
...
def get_performance(mod, xvar, yvar, runtype,threshhold, show):
  ...
  if is_regressor(mod):
    ...
    mod_pred = convert_ratio(mod_pred_proba)
      ...
      if threshhold>0.5:
        print('Threshold used: ',threshhold)
        mod_pred=(mod.predict_proba(xvar)[:, 1] >= threshhold).astype(int)
  ...
  TN = 0
  FN = 0
  for i in range(len(mod_pred)): 
    if yvar[i]==mod_pred[i]==1:
      TP += 1
    if mod_pred[i]==1 and yvar[i]!=mod_pred[i]:
      FP += 1
    if yvar[i]==mod_pred[i]==0:
      TN += 1
    if mod_pred[i]==0 and yvar[i]!=mod_pred[i]:
      FN += 1

  #print(f'True Negative Rate:{TN/(TN+FP)} {runtype}')
